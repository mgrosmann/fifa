...
# --- Charger DB FIFA ---
fifa_db = pd.read_csv(fifa_csv, sep=";")  # <-- utiliser ; ici
...
df = pd.read_csv(csv_file, sep=";", quotechar='"')  # <-- utiliser ; ici aussi
...
for _, row in df.iterrows():
    ...
    row["source"] = os.path.basename(csv_file)  # garder le nom du fichier comme source
    if pid in fifa_ids:
        joueurs_deja += 1
...
# --- Export CSV ---
pd.DataFrame(nouveaux).to_csv(output_new_file, index=False, sep=";")
pd.DataFrame(existants).to_csv(output_existing_file, index=False, sep=";")
